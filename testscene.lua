...
function TestScene:init()
	...
	self.snake:updateFood()
	self.snake.ai3 = SnakeAIBFSTail()
...
TestScene.drawBox = drawBox

-- 绘制一个格子
function TestScene:drawBlock(x,y,p,d)
...
-- 测试画路径的方格
function TestScene:drawdirblock(x,y,num)
	local x = self.pos.x + (x-1) * self.unitsize
	local y = self.pos.y + (y-1) * self.unitsize
	...
	lg.rectangle('line', x+3, y+3, size-6, size-6)
	lg.setColor(r, g, b, a)
	local text = {{0,0,0,100},tostring(num)}
	lg.print(text,x,y,0,2,2)
	
end
-- 得到对应的quadID，p为位置（头部，身体，尾巴），d为方向
function TestScene:getImgID(p,d)
	if p == 'head' then
		if d == 'up' then
			return 1
		elseif d == 'right' then
...
function TestScene:draw()
    ...
    self:drawSnake()
    local num = 0
    if self.shortdir then
    	for  i = self.shortdir.head,self.shortdir.tail + 1,-1 do
    		num = num + 1
    		self:drawdirblock(self.shortdir.list[i].x,self.shortdir.list[i].y,num)
    	end
    end
    num = 0
    if self.expdir then
    	for  i = self.expdir.head,self.expdir.tail + 1,-1 do
    		num = num + 1
    		self:drawdirblock(self.expdir.list[i].x,self.expdir.list[i].y,num)
    	end
    end
    self:drawState()
...
function TestScene:keypressed(key)
    ...
    if key == 's' then -- 按s获得最短路径。
    	local list = self.snake.list
		local h = {x = list[list.head].x,y = list[list.head].y}
		local t = {x = list[list.tail].x,y = list[list.tail].y}
    	self.shortdir = self.snake.ai3:getShortestPath(h,t,self.snake)
    elseif key == 'e' then -- 按e扩展路径
    	if self.shortdir.length > 0 then
    		self.expdir = self.snake.ai3:expand(self.shortdir,self.snake)
    	end
    ...
    elseif key == 'f' then
		...
		local h = {x = list[list.head].x,y = list[list.head].y}
		local food = self.snake.food
    	self.shortdir = self.snake.ai3:getShortestPath(h,food,self.snake)
    elseif key == 'up' then
        self.snake:turn('up')
